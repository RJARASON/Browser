...
const buildRadialChart = (props) => {
  ...
  g
    ...
    .attr('fill-opacity', (d) => ((policy_values[d.category].indexOf(d.value) + 1) / policy_values[d.category].length) * .9);
...
pm.getData((_err, data) => {
  buildRadialChart(data);
});
</script>
<div id="chart"></div>
`;

const { states, issues } = pm.response.json().data;
pm.visualizer.set(template, { states: states.edges, issues: issues.edges });